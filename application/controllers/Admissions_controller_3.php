...
class Admissions_controller_3 extends CI_Controller {

	public function __construct()
	{
		parent::__construct();
		$this->load->model('admissions_model_3','person');
	...
	public function index()
	{
      ...
      $this->load->model('admissions_model_3','admission');
	  ...
	  $this->load->view('backend/secondary/admissions/form3/Admissions',$data);
	}
    ...
    public function upload() {
      ...
      $data['logo']=$this->session->userdata('logo');
  	  ...
  	  $this->load->view('backend/secondary/admissions/form3/uploadExcel',$data);
    ...
    public function add()
 	{
 		 ...
 		 if($this->input->post('userSubmit')){
            ...
            if(!empty($_FILES['contacts']['name'])){
        		...
        		redirect(base_url().'uploadAdmission3');
            }
         }
      ...
      $objPHPExcel = PHPExcel_IOFactory::load($storagename);
		...
		$arrayCount = count($allDataInSheet);  // Here get total count of row in that Excel sheet
 		...
 		for($i=2;$i<=$arrayCount;$i++){
        	...
        	$this->load->database();
        	$this->load->model('admissions_model_3');
   			$this->admissions_model_3->add_data($data_user);
  			}

        	$this->session->set_flashdata('msg','<div class="alert alert-success text-center">Data Successfully Uploaded</div>');
        		redirect(base_url().'admission3');
		 }
