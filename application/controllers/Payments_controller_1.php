...
//fee balance
class Payments_controller_1 extends CI_Controller {

	public function __construct()
	{
		parent::__construct();
		$this->load->model('payments_model_1','payment');
	...
	public function index()
	{
	  ...
	  $this->load->view('backend/secondary/payments/form1/Payments',$data);
	}
    ...
    public function upload() {
      ...
      $data['logo']=$this->session->userdata('logo');
  	  ...
  	  $this->load->view('backend/secondary/payments/form1/uploadExcel',$data);
            ...
            if(!empty($_FILES['payments']['name'])){
                ...
                if($this->upload->do_upload('payments')){
                 ...
                 unlink("assets/uploads/secondary/".$file);
        			...
        			redirect(base_url().'uploadPayments1');
            }
         }
      ...
      $objPHPExcel = PHPExcel_IOFactory::load($storagename);
		...
		$arrayCount = count($allDataInSheet);  // Here get total count of row in that Excel sheet
 		...
 		for($i=2;$i<=$arrayCount;$i++){
        	...
        	$this->load->database();
        	$this->load->model('payments_model_1');
   			$this->payments_model_1->add_data($data_user);
  			}
