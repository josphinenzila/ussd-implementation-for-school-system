...
class Exams_controller_4 extends CI_Controller {

	public function __construct()
	{
		parent::__construct();
		$this->load->model('exams_model_4','exam');
	...
	public function index()
	{
	  ...
	  $this->load->view('backend/secondary/exams/form4/Exams',$data);
	}
    ...
    public function upload() {
      ...
      $data['logo']=$this->session->userdata('logo');
  	  ...
  	  $this->load->view('backend/secondary/exams/form4/uploadExcel',$data);
    ...
    public function add()
 	{
 		 ...
 		 if($this->input->post('userSubmit')){
            ...
            if(!empty($_FILES['exams']['name'])){
        		...
        		redirect(base_url().'uploadExams4');
            }
         }
      ...
      $objPHPExcel = PHPExcel_IOFactory::load($storagename);
		...
		$arrayCount = count($allDataInSheet);  // Here get total count of row in that Excel sheet
 		...
 		for($i=2;$i<=$arrayCount;$i++){
        	...
        	$this->load->database();
        	$this->load->model('exams_model_4');
   			$this->exams_model_4->add_data($data_user);
  			}
