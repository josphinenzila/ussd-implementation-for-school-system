...
class Admissions_controller_1 extends CI_Controller {

	public function __construct()
	{
		parent::__construct();
		$this->load->model('admissions_model_1','person');
	...
	public function index()
	{
      ...
      $this->load->model('admissions_model_1','admission');
	  ...
	  $this->load->view('backend/secondary/admissions/form1/Admissions',$data);
	}
    ...
    public function upload() {
      ...
      $data['logo']=$this->session->userdata('logo');
  	  ...
  	  $this->load->view('backend/secondary/admissions/form1/uploadExcel',$data);
 		 ...
 		 if($this->input->post('userSubmit')){
            ...
            if(!empty($_FILES['contacts']['name'])){
                ...
                if($this->upload->do_upload('contacts')){
                 ...
                 //s$storagename = "assets/uploads/secondary/Contacts.xlsx";
                 move_uploaded_file($_FILES["contacts"]["tmp_name"],  $file);
               ...
               $this->session->set_flashdata('msg','<div class="alert alert-danger text-center">Please select an Excel File</div>');
        		redirect(base_url().'uploadAdmission1');
            }
         }
      ...
      $objPHPExcel = PHPExcel_IOFactory::load($file);
		...
		$arrayCount = count($allDataInSheet);  // Here get total count of row in that Excel sheet
 		...
 		for($i=2;$i<=$arrayCount;$i++){
        	...
        	$this->load->database();
        	$this->load->model('admissions_model_1');
   			$this->admissions_model_1->add_data($data_user);
  			}

        	$this->session->set_flashdata('msg','<div class="alert alert-success text-center">Data Successfully Uploaded</div>');
        		redirect(base_url().'admission1');
		 }
