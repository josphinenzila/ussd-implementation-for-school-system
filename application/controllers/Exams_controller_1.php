...
class Exams_controller_1 extends CI_Controller {

	public function __construct()
	{
		parent::__construct();
		$this->load->model('exams_model_1','exam');
	...
	public function index()
	{
	  ...
	  $this->load->view('backend/secondary/exams/form1/Exams',$data);
	}
    ...
    public function upload() {
      ...
      $data['logo']=$this->session->userdata('logo');
  	  ...
  	  $this->load->view('backend/secondary/exams/form1/uploadExcel',$data);
 		 ...
 		 if($this->input->post('userSubmit')){
            ...
            if(!empty($_FILES['exams']['name'])){
               ...
               $this->session->set_flashdata('msg','<div class="alert alert-danger text-center">Please select an Excel File</div>');
        		redirect(base_url().'uploadExams1');
            }
         }
       ...
       $objPHPExcel = PHPExcel_IOFactory::load($storagename);
		...
		$arrayCount = count($allDataInSheet);  // Here get total count of row in that Excel sheet
 		...
 		for($i=2;$i<=$arrayCount;$i++){
        	...
        	$this->load->database();
        	$this->load->model('exams_model_1');
   			$this->exams_model_1->add_data($data_user);
  			}
