...
class Admissions_controller_4 extends CI_Controller {

	public function __construct()
	{
		parent::__construct();
		$this->load->model('admissions_model_4','person');
	...
	public function index()
	{
      ...
      $this->load->model('admissions_model_4','admission');
	  ...
	  $this->load->view('backend/secondary/admissions/form4/Admissions',$data);
	}
    ...
    public function upload() {
      ...
      $data['logo']=$this->session->userdata('logo');
  	  ...
  	  $this->load->view('backend/secondary/admissions/form4/uploadExcel',$data);
 		 ...
 		 if($this->input->post('userSubmit')){
            ...
            if(!empty($_FILES['contacts']['name'])){
                ...
                if($this->upload->do_upload('contacts')){
                 ...
                 move_uploaded_file($_FILES["contacts"]["tmp_name"],  $storagename);
        		...
        		redirect(base_url().'uploadAdmission4');
            }
         }
       ...
       $objPHPExcel = PHPExcel_Settings::setZipClass(PHPExcel_Settings::PCLZIP);
		...
		$arrayCount = count($allDataInSheet);  // Here get total count of row in that Excel sheet
 		...
 		for($i=2;$i<=$arrayCount;$i++){
        	...
        	$this->load->database();
        	$this->load->model('admissions_model_4');
   			$this->admissions_model_4->add_data($data_user);
  			}

        	$this->session->set_flashdata('msg','<div class="alert alert-success text-center">Data Successfully Uploaded</div>');
        		redirect(base_url().'admission4');
		 }
